...
/**
 * Create fetch options for using a proxy with Axios
 */
export function createProxyConfig(proxyConfig: ProxyConfig): any {
  const { host, port, username, password } = proxyConfig;
  
  const axiosProxyConfig: any = {
    host,
    port
  };
...
export function createProxyFetchOptions(
  ...
  // Since Node's fetch doesn't natively support proxies, in production this would use
  // a library like 'global-agent' or 'node-fetch-with-proxy'
  
  console.log(`Using proxy ${formatProxyUrl(proxyConfig)} for request`);
  
  return {
    ...options,
    // Add a header to indicate we're using a proxy (for reference only)
    headers: {
...
export async function testProxy(proxyConfig: ProxyConfig): Promise<{ working: boolean, latency: number, error?: string }> {
  ...
  try {
    ...
    // This would require using a library that supports proxies like axios with a proxy agent
    
    console.log(`Testing proxy ${formatProxyUrl(proxyConfig)}`);
    
    throw new Error('Proxy testing requires HTTP client configuration. Please implement proxy testing with your HTTP client of choice.');
