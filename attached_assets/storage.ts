...
export class MemStorage implements IStorage {
  ...
  async deleteScrapedProducts(configId: number): Promise<boolean> {
    const productIds = Array.from(this.scrapedProducts.entries())
      .filter(([_, product]) => product.configId === configId)
      .map(([id]) => id);
      
    for (const id of productIds) {
      this.scrapedProducts.delete(id);
    }
  ...
  async createScraperJob(job: InsertScraperJob): Promise<ScraperJob> {
    ...
    const newJob: ScraperJob = {
      ...
      startTime: new Date(),
      endTime: null,
      itemsScraped: job.itemsScraped || null,
